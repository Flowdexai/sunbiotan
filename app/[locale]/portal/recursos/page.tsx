...
export default function RecursosPage() {
  const t = useTranslations('PortalRecursos');
  ...
  const categoryConfig = [
    { key: 'formacao', label: t('catFormacao'), icon: Play, description: t('catFormacaoDesc') },
    { key: 'marketing', label: t('catMarketing'), icon: ImageIcon, description: t('catMarketingDesc') },
    { key: 'marca', label: t('catMarca'), icon: Palette, description: t('catMarcaDesc') },
  ];
    ...
    <div className="space-y-6">
        ...
        {categoryConfig.map(({ key, label, icon: Icon, description }) => {
          ...
          return (
            <button
              ...
              className={`p-4 rounded-xl border text-left transition-all ${isActive ? 'border-sunbiotan-500 bg-sunbiotan-50 shadow-sm' : 'border-sunbiotan-100 bg-white hover:border-sunbiotan-300'}`}
            >
              <div className="flex items-center gap-3 mb-2">
                <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${isActive ? 'bg-sunbiotan-500' : 'bg-sunbiotan-100'}`}>
                  <Icon className={`h-4 w-4 ${isActive ? 'text-white' : 'text-sunbiotan-600'}`} />
                </div>
                <span className={`text-sm font-medium ${isActive ? 'text-sunbiotan-900' : 'text-sunbiotan-700'}`}>
                  {label}
                </span>
              </div>
              <p className="text-xs text-sunbiotan-500 font-light">
                {count} {count === 1 ? t('resource') : t('resources')}
              </p>
            </button>
          );
        })}
      ...
      {filtered.length === 0 ? (
        <div className="bg-white rounded-xl border border-sunbiotan-100 p-12 text-center">
          <p className="text-sunbiotan-500 text-sm font-light">{t('noResources')}</p>
          <p className="text-sunbiotan-400 text-xs mt-1">{t('comingSoon')}</p>
        </div>
      ) : (
              ...
              >
                ...
                <div className="p-5">
                  <div className="flex items-start justify-between gap-3 mb-2">
                    <h3 className="text-sm font-medium text-sunbiotan-900">{resource.title}</h3>
                  ...
                  {resource.description && (
                    <p className="text-xs text-sunbiotan-500 font-light mb-4 line-clamp-2">{resource.description}</p>
                  )}
