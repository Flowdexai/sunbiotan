...
export default function EncomendasPage() {
  const t = useTranslations('PortalEncomendas');
  ...
  const statusConfig = {
    pending: { label: t('statusPending'), icon: Clock, color: 'bg-amber-100 text-amber-700' },
    confirmed: { label: t('statusConfirmed'), icon: CheckCircle, color: 'bg-blue-100 text-blue-700' },
    shipped: { label: t('statusShipped'), icon: Truck, color: 'bg-purple-100 text-purple-700' },
    delivered: { label: t('statusDelivered'), icon: Package, color: 'bg-green-100 text-green-700' },
  };
  ...
  useEffect(() => {
    async function load() {
      ...
      const { data } = await supabase
        .from('orders')
        .select(`id, status, message, created_at, order_items ( id, quantity, products ( name ) )`)
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sunbiotan-600 text-sm font-light">
            {orders.length === 1 ? t('count', { count: 1 }) : t('countPlural', { count: orders.length })}
          </p>
        </div>
        <Link
      ...
      {orders.length === 0 ? (
        <div className="bg-white rounded-xl border border-sunbiotan-100 p-12 text-center">
          ...
          <p className="text-sunbiotan-600 text-sm font-light mb-4">{t('noOrders')}</p>
          <Link
            href="/portal/encomendas/nova"
        ...
        <div className="space-y-4">
          {orders.map((order) => {
            ...
            return (
              <div key={order.id} className="bg-white rounded-xl border border-sunbiotan-100 p-5 shadow-sm">
                <div className="flex items-center justify-between mb-4">
                  <div>
                    ...
                    <p className="text-xs text-sunbiotan-300 mt-0.5">#{order.id.slice(0, 8).toUpperCase()}</p>
                  </div>
                ...
                <div className="space-y-2 mb-4">
                  {order.order_items.map((item) => (
                    <div key={item.id} className="flex items-center justify-between text-sm">
                      ...
                      <span className="text-sunbiotan-500 text-xs">{t('qty', { qty: item.quantity })}</span>
                    </div>
                  ))}
                </div>
                ...
                {order.message && (
                  <div className="bg-sunbiotan-50 rounded-lg px-4 py-3">
                    <p className="text-xs text-sunbiotan-600 font-light">{order.message}</p>
                  </div>
                )}
              </div>
            );
